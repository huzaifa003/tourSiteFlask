...
engine = create_engine("mysql+mysqlconnector://root:@localhost/tour",echo=True)
con = engine.connect()
...
def login():
    if (request.method == "GET"):
        ...
        result = con.execute(text("SELECT * FROM users WHERE email = (:mail)  and password = (:password)"),{"mail" : email, "password" : password})
...
def signup():
    if ('user' in session):
        ...
        if (request.method == "GET"):
            ...
            result = con.execute(text("SELECT * FROM users WHERE email = (:mail) "),{"mail" : email})
            data = convertResult(result)
            if (len(data) > 0):
                return "THIS EMAIL ALREADY EXISTS"
            
            con.execute(text("INSERT INTO users (email,password) VALUES (:mail, :password)"),{"mail":email, "password" : password})
            return redirect("/login")
...
def bookingDetails():
    if request.method == 'GET':
        ...
        if ("add" in request.form):
            ...
            con.execute(text("INSERT INTO trips (source, destination, source_time, destination_time, price) VALUES (:source, :destination, :source_time, :destination_time, :price)"),{"source": source, "destination" : destination, "source_time" : source_time, "destination_time" : destination_time, "price" : price})
...
def editBookings(id):
    if request.method == "GET":
        if (session['id'] == 0):
            print(id)
            result  = con.execute(text("SELECT * FROM trips WHERE id = (:id) "), {"id": id})
            converted = convertResult(result)
            for i in converted:
        ...
        print(source,id)
        con.execute(text("UPDATE trips SET source = (:source), destination = (:destination), source_time = (:source_time), destination_time = (:destination_time), price = (:price) WHERE id = (:id)"),{"source": source, "destination" : destination, "source_time" : source_time, "destination_time" : destination_time, "price" : price, "id":id})
